...
def testing(img_input):
    ...
    bow_descs = np.load('index.npy').astype(np.float32) #load
...
for folder in test_folders: #παιρνω τα αρχεια καθε φακελου ..λιστα απο 97 στοιχει αμε τ α ονοματα των φακελων
    files = os.listdir(folder) #φακελοσ / file
    for file in files:
